...
interface LandingPageProps {
  onStoryboardGenerated: (storyboard: Storyboard, outputType: OutputType, style?: string) => void
}

export default function LandingPage({ onStoryboardGenerated }: LandingPageProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Cannot connect to backend: ${connError.message}. Make sure the backend server is running on port 3000.`)
      }

      // Step 1: Scrape the website/GitHub repo
      console.log('Scraping website...', githubUrl)
      const scrapeResult = await api.scrapeWebsite(githubUrl)
      console.log('Scrape result:', scrapeResult)
      
      // Step 2: If output is slides, generate them
      if (outputType === 'slides') {
        // Map style to backend style (YC or Finance)
        const backendStyle = style === 'vc-pitch' ? 'YC' : 'Finance'
        
        console.log('Generating slides...', { style: backendStyle })
        const slidesResult = await api.generateSlides(scrapeResult.storyboard, backendStyle)
        console.log('Slides result:', slidesResult)
        
        setPresentationUrl(slidesResult.presentationUrl)
        
        // Open presentation in new tab
        if (slidesResult.presentationUrl) {
          window.open(slidesResult.presentationUrl, '_blank')
        }
      }
      
      // Navigate to storyboard view (always show storyboard, even if slides were generated)
      onStoryboardGenerated(scrapeResult.storyboard, outputType, style)
      
    } catch (err: any) {
      console.error('Generation error:', err)
      // Provide more helpful error messages
      let errorMessage = err.message || 'Failed to generate presentation'
      
      if (errorMessage.includes('AGENT_API_KEY')) {
        errorMessage = 'Backend API key not configured. Please check backend/.env file and restart the backend server.'
      } else if (errorMessage.includes('Cannot connect')) {
        errorMessage = 'Cannot connect to backend. Make sure the backend server is running: cd backend && npm run dev'
      } else if (errorMessage.includes('500')) {
        errorMessage = 'Backend server error. Check the backend console for details.'
      }
    ...
    <div className="min-h-screen relative overflow-hidden bg-background flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-2xl space-y-4">
          ...
          {presentationUrl && (
            <div className="mt-4 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-500 text-sm">
              ✅ Presentation ready! <a href={presentationUrl} target="_blank" rel="noopener noreferrer" className="underline">Open in new tab</a>
            </div>
          )}
